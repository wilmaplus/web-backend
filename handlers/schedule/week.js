...
function schedule(req, res) {
    try {
        if (req.body.session && req.body.server && req.body.date) {
            ...
            if (date.length > 64) {
                resUtils.responseStatus(res, 400, false, {cause: 'date is too long!'});
                return;
            }
            if (isNaN(new Date(date))) {
                resUtils.responseStatus(res, 400, false, {cause: 'Invalid date!'});
                return;
            }
            ...
            wilmaApi.scheduleWithDate(new Date(date), session, server, function (error, localization) {
                if (localization)
